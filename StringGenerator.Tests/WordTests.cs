...
namespace StringGenerator.Tests
{
    ...
    public class Tests
    {
        ...
        {
            var word = new Word(Chunk.Create<char>(new char[] { 'a', 'b' }), Chunk.Create<int>(new int[] { 1, 2, 3 }));
        ...
        {
            var word = new Word(Chunk.Create(1, 2), Chunk.Create(3, 4));
        ...
        public void Increment_GoesThrowAllFileValues_WhenCalls()
        {
            using(var tempFile1 = new TempFile())
            {
                using (var tempFile2 = new TempFile())
                {
                    ...
                    var word = new Word(Chunk.Create(tempFile1.FileName), Chunk.Create(tempFile2.FileName));
